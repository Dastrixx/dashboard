...
Если запросы 1С заканчиваются таймаутом, запустите с той же машины,
где работает Node.js-сервер, `npm run probe:onec`. Проверка выполняет
три коротких последовательных запроса: одну запись справочника, заголовок
отчёта и отчёт с товарами и сортировкой. Для диагностики используется
таймаут 10 секунд на запрос (можно задать `ONEC_PROBE_TIMEOUT_MS`);
ответы и учётные данные в консоль не выводятся. Если уже первый запрос
зависает, проверьте доступность OData/сервера 1С и работу самой базы.
